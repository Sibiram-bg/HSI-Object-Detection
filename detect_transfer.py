...
# ... (The rest of the script is the same as your corrected detect.py) ...
...
def detect(tiff_image_path, min_score, max_overlap, top_k, suppress=None):
    ...
    for i in range(det_boxes.size(0)):
        ...
        text_width = text_bbox[2]; text_height = text_bbox[3]
        box_top = box_location[1]
        text_y_position = box_top - text_height - 2
        if text_y_position < 0: text_y_position = box_top + 2
...
if __name__ == '__main__':
    ...
    # --- MODIFIED: Saves to a new folder ---
    if not os.path.exists('results_transfer'):
        os.makedirs('results_transfer')
    ...
    for i, img_path in enumerate(image_paths):
        ...
        if os.path.exists(local_img_path):
            ...
            if annotated_image:
                ...
                # --- MODIFIED: Saves to a new folder ---
                annotated_image.save(f'./results_transfer/{base_filename}.png')
        else:
